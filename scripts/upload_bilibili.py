...
Runs on Colab (preferred) or local Mac. Reads cookies from Google Drive.

Usage: python3 upload_bilibili.py <folder> --title "标题" --desc "描述" --tags "标签1,标签2"
"""
import os, sys, json, re, time, subprocess

# Auto-detect: Colab or local macOS
_COLAB_DRIVE = '/content/drive/MyDrive/video-translate'
...
if os.path.exists(_COLAB_DRIVE):
    ...
    for _cand in _candidates:
        _base = os.path.join(_cloud, _cand)
...
def generate_p2(folder):
    ...
    vf = f"subtitles='{srt_esc}':force_style='FontName=Noto Sans CJK SC,FontSize=16'"
    r = subprocess.run(
...
def get_source(folder):
    """Get source/uploader from download.info.json"""
    info_path = os.path.join(folder, 'download.info.json')
    if os.path.exists(info_path):
        with open(info_path, 'r', encoding='utf-8') as f:
            info = json.load(f)
        return info.get('uploader', info.get('channel', 'YouTube'))
    return 'YouTube'


def upload(folder, title, desc, tags):
    """Upload video with P1 (dubbed) + P2 (original+subs) to Bilibili.
    title, desc, tags are REQUIRED — Claude must provide them.
    """
    from biliup.plugins.bili_webup import BiliBili, Data

    if not title or not desc or not tags:
        print('Error: --title, --desc, --tags are all required.')
        return False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--title', required=True, help='Video title (Claude writes this)')
    parser.add_argument('--desc', required=True, help='Video description (Claude writes this)')
    parser.add_argument('--tags', required=True, help='Comma-separated tags (Claude writes this)')
